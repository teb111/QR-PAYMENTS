...
export const initialisePaymentAction = (data) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.post(`/initialise_payment`, paymentData, config);
        dispatch({
...
export const verifyTransactionAction = (transaction_ref) => async (dispatch) => {
    try {
        dispatch({
        ...
        const { data } = await axios.post(`/verify_payment`, { transaction_ref }, config);
        dispatch({
