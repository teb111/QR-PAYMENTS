...
function Details({ handleClick }) {
  ...
  }, [card, id, dispatch, loading, success, error, transaction, transactionRef])
          ...
          {!loading && success && transaction?.status === "success" ? (
            <>
              <MerchantCard
                ...
                bank={bank}
                Confirm={() =>
                  exitScreen(wrapper, handleClick, PAYMENT_STEP.AMOUNT)
                }
              />
              <p
                ref={(el) => {
              ...
              >
                ...
                the details above are correct and tap “ppm roceed to payment”
              </p>
            </>
          ) : null}
        </div>
